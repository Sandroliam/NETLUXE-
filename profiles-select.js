...
function nxBuildProfileScreen(){
  ...
  d.innerHTML =
    '<div class="nxsel-in">'+
      ...
      '</div>'+
      '<h1 class="nxsel-t">Qui regarde&nbsp;?</h1>'+
      '<p class="nxsel-d">Chaque profil garde son historique et ses préférences</p>'+
      '<div class="nxsel-grid" id="nxselGrid"></div>'+
      '<div class="nxsel-act">'+
        '<button class="btn btn-o" onclick="nxOpenCreate()">+ Ajouter un profil</button>'+
        '<button class="btn btn-o" style="color:var(--nx-coral,#FF6B35)" onclick="logout()">Déconnexion</button>'+
      '</div>'+
    '</div>';
  ...
  m.innerHTML =
    '<div class="nxcr-box">'+
      ...
      '<div class="nxcr-body">'+
        '<div class="nxcr-prev">'+
          '<div class="nxcr-av" id="nxcrAv" onclick="document.getElementById(\'nxcrPhoto\').click()">?</div>'+
          '<input type="file" id="nxcrPhoto" accept="image/*" style="display:none" onchange="nxcrPhoto(event)">'+
          '<div class="nxcr-photo-act">'+
            '<button class="btn btn-o" onclick="document.getElementById(\'nxcrPhoto\').click()">📷 Photo</button>'+
            '<button class="btn btn-o" onclick="nxcrClearPhoto()">Retirer</button>'+
          '</div>'+
        '</div>'+
        ...
        '<label class="nxcr-lb">Couleur</label>'+
        '<div class="nxcr-colors" id="nxcrColors"></div>'+
...
function nxRenderProfileGrid(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    var av = p.avatar
      ? '<div class="nxsel-av" style="background-image:url('+p.avatar+')"></div>'
      : '<div class="nxsel-av" style="background:'+(p.color||ty.color)+'">'+
        (p.name||'?').charAt(0).toUpperCase()+'</div>';
    h += '<div class="nxsel-card" onclick="nxPickProfile(\''+p.id+'\')">'+
      av+
      '<div class="nxsel-nm">'+nxEsc(p.name)+'</div>'+
      '<div class="nxsel-ty"><span>'+ty.ic+'</span>'+ty.label+'</div>'+
      '<button class="nxsel-edit" onclick="event.stopPropagation();nxOpenEdit(\''+p.id+'\')" title="Modifier">✎</button>'+
    '</div>';
  }
  if(list.length < 5){
    h += '<div class="nxsel-card add" onclick="nxOpenCreate()">'+
